...
bool Handle_S_OFFICE_SET_AUTHORITY(shared_ptr<GameSession>& session, Protocol::S_OFFICE_SET_AUTHORITY& pkt);
...
public:
	static void Init()
	{
		...
		GPacketHandler[PKT_S_OFFICE_SET_AUTHORITY] = [](shared_ptr<GameSession>& session, unsigned char* buffer, int len) { return HandlePacket < Protocol::S_OFFICE_SET_AUTHORITY > (Handle_S_OFFICE_SET_AUTHORITY, session, buffer, len); };
	...
	static shared_ptr<SendBuffer> MakeSendBuffer(Protocol::C_OFFICE_SET_AUTHORITY& pkt) { return MakeSendBuffer(pkt, PKT_C_OFFICE_SET_AUTHORITY); }
