...
public:
	static void Init()
	{
		...
		GPacketHandler[PKT_S_WILDCARD] = [](shared_ptr<GameSession>& session, unsigned char* buffer, int len) { return HandlePacket < Protocol::S_WILDCARD > (Handle_S_WILDCARD, session, buffer, len); };
		GPacketHandler[PKT_S_WILDCARD_MAP] = [](shared_ptr<GameSession>& session, unsigned char* buffer, int len) { return HandlePacket < Protocol::S_WILDCARD_MAP > (Handle_S_WILDCARD_MAP, session, buffer, len); };
		...
		GPacketHandler[PKT_S_MATCHING_AWARD] = [](shared_ptr<GameSession>& session, unsigned char* buffer, int len) { return HandlePacket < Protocol::S_MATCHING_AWARD > (Handle_S_MATCHING_AWARD, session, buffer, len); };
	...
	static shared_ptr<SendBuffer> MakeSendBuffer(Protocol::C_WILDCARD& pkt) { return MakeSendBuffer(pkt, PKT_C_WILDCARD); }
	static shared_ptr<SendBuffer> MakeSendBuffer(Protocol::C_WILDCARD_MAP& pkt) { return MakeSendBuffer(pkt, PKT_C_WILDCARD_MAP); }
	...
	static shared_ptr<SendBuffer> MakeSendBuffer(Protocol::C_BASE_REMOVE_OBJECT_BY_ID& pkt) { return MakeSendBuffer(pkt, PKT_C_BASE_REMOVE_OBJECT_BY_ID); }
