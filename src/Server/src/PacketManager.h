...
public:
	static void Init()
	{
		...
		GPacketHandler[PKT_C_OFFICE_SET_AUTHORITY] = [](shared_ptr<GameSession>& session, unsigned char* buffer, int len) { return HandlePacket < Protocol::C_OFFICE_SET_AUTHORITY > (Handle_C_OFFICE_SET_AUTHORITY, session, buffer, len); };
	...
	static shared_ptr<SendBuffer> MakeSendBuffer(Protocol::S_OFFICE_SET_AUTHORITY& pkt) { return MakeSendBuffer(pkt, PKT_S_OFFICE_SET_AUTHORITY); }
