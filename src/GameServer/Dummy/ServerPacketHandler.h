...
bool Handle_S_BASE_SET_SCENE(shared_ptr<GameSession>& session, Protocol::S_BASE_SET_SCENE& pkt);
...
public:
	static void Init()
	{
		...
		GPacketHandler[PKT_S_BASE_SET_SCENE] = [](shared_ptr<GameSession>& session, unsigned char* buffer, int len) { return HandlePacket<Protocol::S_BASE_SET_SCENE>(Handle_S_BASE_SET_SCENE, session, buffer, len); };
	...
	static shared_ptr<SendBuffer> MakeSendBuffer(Protocol::C_BASE_SET_SCENE& pkt) { return MakeSendBuffer(pkt, PKT_C_BASE_SET_SCENE); }
